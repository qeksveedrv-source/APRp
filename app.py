...
with st.container():
    ...
    with col_a: addr = st.text_input("輸入目標地址", "花蓮市中正路")
    with col_b: b_type = st.selectbox("建物型態", ["透天厝", "住宅大樓(11層含以上有電梯)", "華廈(10層含以下有電梯)", "公寓(5樓含以下無電梯)"])
    
    if b_type == "透天厝":
        c1, c2, c3 = st.columns(3)
        with c1: land_area = st.number_input("土地面積 (坪)", min_value=0.0, value=30.0, step=0.1)
        with c2: land_price = st.number_input("土地行情 (萬/坪)", min_value=0.0, value=14.0, step=0.1)
        with c3: build_area = st.number_input("建物總面積 (坪)", min_value=0.0, value=60.0, step=0.1)
        c4, c5 = st.columns(2)
        with c4: material = st.selectbox("主要建材", ["鋼筋混凝土", "鋼筋混凝土加強磚造"])
        with c5: age = st.number_input("屋齡 (年)", min_value=0, value=20)
        is_first_floor = True 
    else:
        c1, c2, c3 = st.columns(3)
        with c1: build_area = st.number_input("建物權狀面積 (坪)", min_value=0.0, value=30.0, step=0.1)
        with c2: age = st.number_input("屋齡 (年)", min_value=0, value=22)
        with c3: parking_type = st.selectbox("預計車位類別", ["無", "平面", "機械"])
...
if run_btn:
    ...
    if loc:
        ...
        if not raw_pool.empty:
            ...
            # --- [核心需求] 一年過濾邏輯，對齊 deal_date[cite: 1] ---
            now = datetime.now()
            ...
            if len(recent_data) >= 10:
                final_pool, valuation_msg = recent_data, "採用近一年成交紀錄進行估價"
            else:
                final_pool, valuation_msg = scored_pool.copy(), "因近一年成交量不足十筆，系統已納入一年以上紀錄以供參考"
            # ------------------------------------------------------
            ...
            st.session_state.valuation_results = {
                ...
                'b_type': b_type, 'build_area': build_area, 'valuation_msg': valuation_msg
            }
        else: st.session_state.valuation_results = "empty"
    else: st.error("❌ 無法定位地址。")
...
if res == "empty":
    ...
    st.info(f"💡 系統提示：{res['valuation_msg']}") # 顯示對應訊息[cite: 1]
    ...
    st.write("### 📋 近鄰成交參考紀錄")
    top_10 = res['top_10']; top_10['dist_m'] = (top_10['dist'] * 1000).astype(int)

    if res['b_type'] == "透天厝":
        top_10['sort_date_only'] = top_10['deal_date'].astype(str).apply(lambda x: x.split('~')[0])
        ...
        display_cols = {
            ...
            'market_premium': '市場溢價係數', 'total_score': '權重分數', 'deal_date': '成交日'
        }
    else:
        top_10 = top_10.sort_values(by='deal_date', ascending=False)
        ...
        display_cols = {
            ...
            'price_10k': '實登價格(萬)', 'total_score': '權重分數', 'deal_date': '成交日'
        }
    ...
    if view_mode == "📱 手機優先 (Top 5 卡片)":
        for i, (_, row) in enumerate(top_10.head(5).iterrows()):
            with st.expander(f"📍 {row['address']} ({row['deal_date']})", expanded=(i==0)):
                if res['b_type'] == "透天厝":
                    c1, c2 = st.columns(2)
                    c1.metric("距離", f"{row['dist_m']} m"); c2.metric("屋齡", f"{row['calc_age']} 年")
                    ...
                    c1, c2 = st.columns(2)
                    c1.metric("距離", f"{row['dist_m']} m"); c2.metric("屋齡", f"{row['calc_age']} 年")
                    st.write(f"🏢 **建物權狀面積**：{row['total_build_area']} 坪")
